...
# And we can generate a C-index, where the better the model the closer it is
# to 1 and the worse it is the closer it is to .5
Cstat(glm_fit)
...
# We'll do this the long way first, so you can see what we're calculating.
p <- predict(glm_fit, newdata=subset(lr_df,select=c(1,2)), type="response")
pr <- prediction(p, lr_df$student_gender)
prf <- performance(pr, measure = "tpr", x.measure = "fpr")

# At this point we already know that hedging/boosting is not predictive
# of student gender, and the plot emphasizes this.
plot(prf)

# For completeness, let's follow through on the last step.
# And we can return the AUC (area under the curve)
# which Brezina (pg. 125) refers to as the C-index

auc <- performance(pr, measure = "auc")
auc <- auc@y.values[[1]]
auc

# Okay, now you know the long way (and you may want to show the plot),
...
# the final negative log-likelihood 814.275450. This value multiplied by two 
...
# probability of choosing the baseline category is the relative
# risk or odds. The relative risk is the right-hand side linear equation
